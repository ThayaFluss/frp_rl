"""
AGaLiTe (Approximate Gated Linear Transformer) implementation.

This module contains the AGaLiTe architecture for efficient attention-based
sequence modeling in RL. Copied from ~/agalite/src_pure/models/agalite.py.

Key features:
- O(N) linear attention (vs O(N^2) for standard transformers)
...
class GRUGatingUnit(nn.Module):
    """GRU Gating Unit used in GTrXL and AGaLiTe.

    Arguments:
        input_dim: Input dimension
        bg: Initial gate bias value. By setting bg > 0 we can explicitly
            initialize the gating mechanism to be close to the identity map.
            This can greatly improve the learning speed and stability since
            it initializes the agent close to a Markovian policy
...
def discounted_sum_parallel(start_state, x, discounts):
    """Parallel discounted sum computation.
...
class AttentionORLiTLayer(nn.Module):
    """ORLiT (Orthogonal Random Linear Transformer) attention layer."""
    input_dim: int
    head_dim: int
    head_num: int
    eta: int
    ...
    def setup(self):
        self.linear_kqvbetagammas = nn.Dense(
            self.head_num * self.head_dim * 5,
            kernel_init=orthogonal(jnp.sqrt(2)),
            bias_init=constant(0.0)
        )
        self.linear_p1p2p3 = nn.Dense(
            self.head_num * self.eta * 3,
            kernel_init=orthogonal(jnp.sqrt(2)),
            bias_init=constant(0.0)
        )
    ...
    def __call__(self, inputs, terminations, last_memory):
        ...
        Args:
            inputs: attention input of shape (cur_seq, input_dim)
            terminations: termination signal of shape (cur_seq,)
            last_memory: (tilde_k_prev, tilde_v_prev, s_prev, tick)
        ...
        Returns:
            ...
            new_memory: (tilde_k, tilde_v, s, new_tick)
        """
        ...
        # Calculate outer product of gamma and p3
        ...
        final_keys = discounted_sum_parallel(
            tilde_k_prev, keys, jnp.expand_dims(discount_gamma, 1)
        )  # (cur_seq, r, head_num, eta * head_dim)
        final_values = discounted_sum_parallel(
            tilde_v_prev, values, jnp.expand_dims(discount_beta, 1)
...
class RecurrentLinearTransformerEncoder(nn.Module):
    """Single layer of the recurrent linear transformer."""
    d_model: int
    d_head: int
    d_ffc: int
    n_heads: int
    ...
    def __call__(self, inputs, terminations, last_memory):
        ...
        attn_out, new_memory = AttentionORLiTLayer(
            input_dim=self.d_model, head_dim=self.d_head, head_num=self.n_heads,
            eta=self.eta, r=self.r, reset_hidden_on_terminate=self.reset_hidden_on_terminate
        )(ln1_out, terminations, last_memory)
        ...
        ffc_out = nn.Sequential([
            nn.Dense(self.d_ffc, kernel_init=orthogonal(jnp.sqrt(2)), bias_init=constant(0.0)),
            jax.nn.relu,
            nn.Dense(self.d_model, kernel_init=orthogonal(jnp.sqrt(2)), bias_init=constant(0.0))
        ])(ln2_out)
...
class AGaLiTe(nn.Module):
    """AGaLiTe (Approximate Gated Linear Transformer) - non-batched version."""
    n_layers: int
    d_model: int
    d_head: int
    d_ffc: int
    ...
    def __call__(self, inputs, terminations, last_memory):
        ...
        Args:
            ...
            last_memory: dict with layer memories
        ...
        for layer in range(1, len(last_memory)+1):
            if layer == 1:  # Use Dense layer for first layer
                encoder = RecurrentLinearTransformerEncoder(
                    ...
                    reset_hidden_on_terminate=self.reset_on_terminate,
                    use_dense=True, name='layer%d' % (layer)
                )
            else:
                encoder = RecurrentLinearTransformerEncoder(
                    d_model=self.d_model, d_head=self.d_head, d_ffc=self.d_ffc,
                    n_heads=self.n_heads, eta=self.eta, r=self.r,
                    reset_hidden_on_terminate=self.reset_on_terminate,
                    use_dense=False, name='layer%d' % (layer)
                )
...
class BatchedAGaLiTe(nn.Module):
    """Batched AGaLiTe for efficient parallel processing across environments."""
    n_layers: int
    d_model: int
    d_head: int
    d_ffc: int
    ...
    def __call__(self, carry, x):
        ...
        Args:
            ...
            x: tuple of (ins, resets)
                ins: T X B X d_model
                resets: T X B

        Returns:
            new_memory: updated batched memory
            outs: T X B X d_model
        """
        rnn_state = carry
        ins, resets = x
        # ins is of shape T X B X d_model, resets is of shape T X B
        # Transpose to B X T for vmap
        ...
        Batch_AGaLiTe = nn.vmap(
            ...
            n_layers=self.n_layers, d_model=self.d_model, d_head=self.d_head,
    ...
    def initialize_carry(batch_size, n_layers, n_heads, d_head, eta, r):
        """Initialize memory state for batch of environments.
        ...
        Args:
            ...
            eta: eta parameter for ORLiT
            r: r parameter for oscillation

        Returns:
            Batched memory dictionary
        """
        memories = []
